...
class Section042_array_hourglass {
	...
	public static void main(String[] args) {
		...
		m = x / 2;	//자바에서 정수를 나누면 소수점은 자동으로 버려지므로 m=2 가 되고 이는 배열의 중간 행이 된다.
		
		for (i = 0; i <= m; i++) {	//m 중간 행을 의미하고 그때까지 반복을 실시한다.
			L--;	//반복마다 1 감소하므로 배열의 마지막 위치가 줄어드는 셈이다.
			for (j = i; j <= L; j++) {
			//i부터 시작과 j++의 의미는 결국 저장될 번지가 대칭을 이루는것과 비슷하다.
			//예를들면, 첫행의 경우 0~4번지면 두번째 행의 경우 j=1이 되고 L--의 결과로 마지막 번지가 줄어들게 되므로 1~3번지가 된다.
				k++;	//저장될 데이터를 1씩 증가
				a[i][j] = k;	//해당 배열의 위치에 데이터 저장
			}
		}
		for (i = m + 1; i <= x - 1; i++) {	//2차원 배열의 첫번째 배열을 제어하는데 중간을 넘었다면(m 초과)
			L--;	//반복마다 1 감소하면 하단의 for문 시작시 j=L로 인해 시작 위치가 당겨지게 되는것이고 모래시계 모양이 된다.
